...
import {
  ...
  type CampaignTemplateLocaleOverlay,
} from "../../src/core/domain/campaignTemplate/schemas.js";
import { validateCampaignTemplateReferences } from "../../src/core/domain/campaignTemplate/validateTemplateReferences.js";
...
type CampaignTemplateLocaleSessionOverlay = NonNullable<NonNullable<CampaignTemplateLocaleOverlay["sessions"]>[string]>;

function getErrorMessage(error: unknown): string {
  return error instanceof Error ? error.message : "unknown error";
}

function safeCampaignTemplatePath(file: string, label: string, errors: string[]): string | null {
  ...
  if (resolvedFile === resolvedDir || !resolvedFile.startsWith(allowedPrefix)) {
    errors.push(`${label} resolves outside campaign templates directory: ${file}`);
    return null;
  }
  return resolvedFile;
}
...
function validateLocaleSessionChecklist(
  overlaySession: CampaignTemplateLocaleSessionOverlay & { readonly checklist?: unknown },
...
function validateLocaleOverlay(
  ...
  template: CampaignTemplateTemplateFile,
...
async function main() {
  ...
  for (const entry of manifest.templates) {
    ...
    const templatePath = safeCampaignTemplatePath(entry.templateFile, `${templateId}.templateFile`, errors);
    ...
    const refErrors = validateCampaignTemplateReferences(template, `${templateId}.template`);
    ...
    for (const [locale, file] of Object.entries(entry.locales)) {
      if (!file) continue;
      const localePath = safeCampaignTemplatePath(file, `${templateId}.locales.${locale}`, errors);
      if (!localePath) continue;

      const rawOverlay = await readJson(localePath, errors, `${templateId}.locales.${locale}`);
      if (!rawOverlay) continue;

      const overlayParse = campaignTemplateLocaleOverlaySchema.safeParse(rawOverlay);
  ...
  if (errors.length > 0) {
    console.error("❌ Campaign template library invalid:");
  ...
  console.log(`✓ Campaign template library valid (${manifest.templates.length} templates) — ${campaignTemplateDir}`);
}

await main().catch((err) => {
  console.error(`❌ Campaign template validation failed: ${getErrorMessage(err)}`);
  process.exit(1);
});
