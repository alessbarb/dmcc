...
const manifestSchemaPath = resolve(process.cwd(), "public/schemas/campaign-template/manifest/v1.json");
const templateSchemaPath = resolve(process.cwd(), "public/schemas/campaign-template/template/v1.json");
const localeSchemaPath = resolve(process.cwd(), "public/schemas/campaign-template/locale/v1.json");
...
async function main() {
  ...
  const manifestJsonSchema = toJSONSchema(campaignTemplateManifestSchema);
  manifestJsonSchema.$schema = "https://json-schema.org/draft/2020-12/schema";
  manifestJsonSchema.$id = "https://dmcc.local/schemas/campaign-template/manifest/v1.json";

  const templateJsonSchema = toJSONSchema(campaignTemplateTemplateFileSchema);
  templateJsonSchema.$schema = "https://json-schema.org/draft/2020-12/schema";
  templateJsonSchema.$id = "https://dmcc.local/schemas/campaign-template/template/v1.json";

  const localeJsonSchema = toJSONSchema(campaignTemplateLocaleOverlaySchema);
  localeJsonSchema.$schema = "https://json-schema.org/draft/2020-12/schema";
  localeJsonSchema.$id = "https://dmcc.local/schemas/campaign-template/locale/v1.json";
  ...
  if (checkOnly && touched.length > 0) {
    console.error("❌ JSON Schemas are out of sync with Zod definitions. Run npm run templates:schema:generate.");
