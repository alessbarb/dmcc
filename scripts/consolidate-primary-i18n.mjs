...
function sectionRange(source, sectionName) {
  const marker = `  ${sectionName}: {\n`;
  const start = source.indexOf(marker);
  if (start < 0) throw new Error(`Missing section ${sectionName}`);
  const contentStart = start + marker.length;
  const rest = source.slice(contentStart);
  const nextSectionMatch = rest.match(/\n  [A-Za-z][A-Za-z0-9]*: \{/);
  const end = nextSectionMatch ? contentStart + nextSectionMatch.index : source.length;
  return { marker, start, contentStart, end };
}

function addEntries(source, sectionName, values, requiredKey) {
  const range = sectionRange(source, sectionName);
  const section = source.slice(range.contentStart, range.end);
  if (section.includes(`    ${requiredKey}:`)) return source;
  const entries = Object.entries(values)
    .map(([key, value]) => `    ${key}: ${render(value, 6)},`)
    .join("\n");
  return `${source.slice(0, range.contentStart)}${entries}\n${source.slice(range.contentStart)}`;
}

for (const [locale, sections] of Object.entries(updates)) {
  const path = `src/shared/i18n/dictionaries/${locale}.ts`;
  let source = readFileSync(path, "utf8");
  source = addEntries(source, "entitiesPage", sections.entitiesPage, "filters");
