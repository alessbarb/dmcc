...
import { getSessionConsequenceChain } from "../../../shared/api/campaignApi.js";
...
export function SessionConsequenceChainPage() {
  ...
  useEffect(() => {
    ...
    void getSessionConsequenceChain(campaignId, sessionId)
      .then(async (response) => {
        ...
        if (!cancelled) setProjection(body.consequenceChain);
      })
      .catch((err: unknown) => {
  ...
  return (
    <div className="session-page session-consequence-chain-page">
      <SessionConsequenceChainCanvas projection={projection} />
    </div>
  );
}
