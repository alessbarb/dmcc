...
export function DmHubQuickActions({
  ...
  const primary = [
    ...
    { icon: <LayoutGrid size={16} />, label: t("landing.quickActionCanvas"), action: onCanvas, primary: true },
    { icon: <Users size={16} />, label: t("landing.quickActionNpcs"), action: onNpcs, primary: true },
    { icon: <Clock size={16} />, label: t("landing.timelineLabel"), action: onTimeline, primary: true },
  ];
  const secondary = [
  ...
  return (
    <nav className="dm-hub-quick-actions" aria-label={t("landing.quickActionsTitle")} data-dm-hub-panel="quick-actions">
      {!onlySecondary && primary.map((item) => (
        <button key={item.label} type="button" className={`dm-hub-quick-actions__btn${item.primary ? " dm-hub-quick-actions__btn--primary" : " dm-hub-quick-actions__btn--secondary"}`} onClick={item.action}>
