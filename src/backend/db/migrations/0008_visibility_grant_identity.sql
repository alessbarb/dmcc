...
--> statement-breakpoint
ALTER TABLE "visibility_grants"
ADD COLUMN "source" text DEFAULT 'visibility' NOT NULL;
--> statement-breakpoint
ALTER TABLE "visibility_grants"
...
--> statement-breakpoint
ALTER TABLE "visibility_grants"
ADD CONSTRAINT "chk_visibility_grants_source"
CHECK ("source" IN ('visibility', 'character_link', 'manual'));
--> statement-breakpoint
CREATE UNIQUE INDEX "uq_visibility_grants_common"
ON "visibility_grants" ("campaign_id", "target_type", "target_id", "scope", "source")
WHERE "scope" NOT IN ('specific_player', 'specific_user');
--> statement-breakpoint
CREATE UNIQUE INDEX "uq_visibility_grants_specific_player"
ON "visibility_grants" ("campaign_id", "target_type", "target_id", "scope", "player_id", "source")
WHERE "scope" = 'specific_player';
--> statement-breakpoint
CREATE UNIQUE INDEX "uq_visibility_grants_specific_user"
ON "visibility_grants" ("campaign_id", "target_type", "target_id", "scope", "user_id", "source")
