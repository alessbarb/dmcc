...
import { hashSecret, verifySecret } from "../web/webSession.js";
import { hashAccessCode } from "./legacyAccessCodes.js";

function allowsLegacyTestAuth(request: any): boolean {
  return request.server?.allowLegacyTestAuth === true;
}

export const allowLegacyTestAuth = allowsLegacyTestAuth;
...
export function assertDmCampaignAccess(request: any, dmSessionSecret: string, dataDir: string, vaultId: string, campaignId: string): string {
...
export function assertCampaignAccess(request: any, state: any, campaignId: string, dmSessionSecret: string, dataDir?: string, vaultId?: string): string {
  const membership = request.unifiedCampaignMembership as { campaignId: string; role: "dm" | "player" | "observer" } | undefined;
  if (membership?.campaignId === campaignId) return membership.role;

  const role = getRequestRole(request, dmSessionSecret);
  if (role === "dm") {
    if (dataDir && vaultId) assertDmCampaignAccess(request, dmSessionSecret, dataDir, vaultId, campaignId);
    return "dm";
  }
  ...
  if (legacyCode && accessCode === legacyCode) return request.headers["x-role"] === "observer" ? "observer" : "player";
  if (hash && hashAccessCode(accessCode) === hash) return request.headers["x-role"] === "observer" ? "observer" : "player";
...
export function getRequestRoleWithTokens(request: any, dmSessionSecret: string, playerTokens: Map<string, { campaignId: string; playerId: string }>, campaignId: string, dataDir?: string, vaultId?: string): "dm" | "player" | "observer" | "unauthenticated" {
  const membership = request.unifiedCampaignMembership as { campaignId: string; role: "dm" | "player" | "observer" } | undefined;
  if (membership?.campaignId === campaignId) return membership.role;

  const dmSession = getRequestDmSession(request, dmSessionSecret);
  if (dmSession) {
    if (dataDir && vaultId && !hasCampaignDmAccessSync(dataDir, vaultId, campaignId, dmSession.dmId)) return "unauthenticated";
    return "dm";
  }
  ...
  if (playerTokenHeader) {
    ...
    if (session && session.campaignId === campaignId) return "player";
  }
  return "unauthenticated";
}
...
export function getRequestPlayerId(request: any): string | undefined {
  ...
  return allowsLegacyTestAuth(request) ? request.headers["x-player-id"] as string | undefined : undefined;
}
