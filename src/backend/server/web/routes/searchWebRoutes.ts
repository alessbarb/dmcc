...
function mapSearchRows(result: unknown, includeDmSummary: boolean) {
  return sqlRows(result).map((row: any) => ({
    ...
    item: {
      ...
      ...(includeDmSummary && row.dm_summary ? { dmSummary: row.dm_summary } : {}),
      visibility: row.visibility,
      entityId: row.entity_id ?? undefined,
      sourceEntityId: row.source_entity_id ?? undefined,
      targetEntityId: row.target_entity_id ?? undefined,
      linkedEntityIds: Array.isArray(row.linked_entity_ids) ? row.linked_entity_ids : undefined,
    },
  }));
}
...
async function runDmSearch(campaignId: string, query: string) {
  ...
  const result = await db.execute(sql`
    SELECT
      ...
      NULL::text AS source_entity_id,
      NULL::text AS target_entity_id,
      NULL::jsonb AS linked_entity_ids
    FROM campaign_entities
    WHERE campaign_id = ${campaignId}
      AND (name ILIKE ${like} OR public_summary ILIKE ${like} OR dm_summary ILIKE ${like})
    ...
    SELECT
      ...
      fact_id AS id,
      kind AS title,
      content_public AS summary,
      content_dm AS dm_summary,
      kind AS visibility,
    ...
    WHERE campaign_id = ${campaignId}
      AND status <> 'archived'
      AND (content_public ILIKE ${like} OR content_dm ILIKE ${like})
    ...
    WHERE campaign_id = ${campaignId}
      AND (type ILIKE ${like} OR public_summary ILIKE ${like} OR dm_summary ILIKE ${like})
    ...
    SELECT
      ...
      public_summary AS summary,
      dm_summary,
      visibility_scope AS visibility,
      entity_id,
      NULL::text AS source_entity_id,
      NULL::text AS target_entity_id,
      NULL::jsonb AS linked_entity_ids
    ...
    WHERE campaign_id = ${campaignId}
      AND status <> 'archived'
      AND (title ILIKE ${like} OR public_summary ILIKE ${like} OR dm_summary ILIKE ${like})
    ...
    LIMIT 80
  `);
  return mapSearchRows(result, true);
}
...
async function runPlayerSearch(campaignId: string, user: WebUser, query: string) {
  ...
  const result = await db.execute(sql`
    SELECT
      ...
      e.entity_id AS id,
      e.name AS title,
      e.public_summary AS summary,
      NULL::text AS dm_summary,
      g.scope AS visibility,
    ...
    LIMIT 80
  `);
  return mapSearchRows(result, false);
}
...
export async function registerSearchWebRoutes(server: FastifyInstance): Promise<void> {
  server.get<{ Params: { campaignId: string }; Querystring: { q?: string } }>(
    ...
    async (request) => {
      ...
      return {
        results: isDmRole(membership.role)
  ...
  server.get<{ Params: { campaignId: string }; Querystring: { q?: string } }>(
    "/api/player/campaigns/:campaignId/search",
    async (request) => {
      const { user, membership } = await requireCampaignMembership(request, request.params.campaignId);
