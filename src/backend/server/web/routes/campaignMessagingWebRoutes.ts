...
export function canReadCampaignMessage(
  ...
  if (message.audience === "dm") return isDmRole(viewer.role);
...
export function campaignMessageSenderColorIndex(senderUserId: string): number {
  let hash = 0;
  for (const character of senderUserId) hash = ((hash << 5) - hash + character.charCodeAt(0)) | 0;
  return Math.abs(hash) % SENDER_COLOR_COUNT;
}
...
function messageVisibilityCondition(viewer: CampaignMessageViewer) {
  ...
  if (isDmRole(viewer.role)) conditions.push(eq(campaignMessages.audience, "dm"));
  if (viewer.playerId) {
    conditions.push(and(
...
async function resolveMessageEventTargets(
  campaignId: string,
  senderUserId: string,
  audience: string,
  recipientPlayerId: string | null,
): Promise<string[] | undefined> {
  if (audience === "party") return undefined;
  if (audience === "player" && recipientPlayerId) {
    const [recipient] = await db.select({ userId: schema.playerProfiles.userId })
      ...
      .limit(1);
    return [...new Set([senderUserId, recipient?.userId].filter((value): value is string => Boolean(value)))];
  }
  const dmMemberships = await db.select({ userId: schema.campaignMemberships.userId })
    .from(schema.campaignMemberships)
    .where(and(
      eq(schema.campaignMemberships.campaignId, campaignId),
      inArray(schema.campaignMemberships.role, ["dm", "co_dm"]),
    ));
  return [...new Set([senderUserId, ...dmMemberships.map((membership) => membership.userId)])];
}
...
export async function registerCampaignMessagingWebRoutes(server: FastifyInstance): Promise<void> {
  server.get<{
    ...
    const visibilityCondition = messageVisibilityCondition({
    ...
    return {
      ...
      messages: pageMessages.map((message) => {
        ...
        return {
          ...
          senderColorIndex: campaignMessageSenderColorIndex(message.senderUserId),
  ...
  server.post<{
    ...
    const visibleMessages = candidates.filter((message) => canReadCampaignMessage(message, {
      role: membership.role,
      userId: user.userId,
      playerId: membership.playerId,
    }));
    if (visibleMessages.length > 0) {
      await db.insert(campaignMessageReads).values(
        visibleMessages.map((message) => ({ messageId: message.messageId, userId: user.userId })),
      ).onConflictDoNothing();
      campaignEventBus.publish(request.params.campaignId, {
        type: "campaign.message.read",
        messageIds: visibleMessages.map((message) => message.messageId),
        targetUserIds: [...new Set(visibleMessages.map((message) => message.senderUserId))],
      });
    }
    reply.code(204);
  });
  ...
  server.post<{
    ...
    campaignEventBus.publish(request.params.campaignId, {
      ...
      targetUserIds: await resolveMessageEventTargets(
