...
async function seedCampaigns(): Promise<void> {
  ...
  await db.insert(schema.campaigns).values([
    { campaignId: ids.campaignA, title: "Campaign A", workspaceId: ids.workspace, ownerId: ids.owner },
    { campaignId: ids.campaignB, title: "Campaign B", workspaceId: ids.workspace, ownerId: ids.owner },
  ]);
...
describe("player visibility referential integrity", () => {
  ...
  it("rejects a membership linked to a player from another campaign", async () => {
    await seedCampaigns();
    await expect(db.insert(schema.campaignMemberships).values({ campaignId: ids.campaignB, userId: ids.viewer, role: "player", playerId: ids.playerA })).rejects.toThrow();
  });
  ...
  it("rejects portal state for a player from another campaign", async () => {
    await seedCampaigns();
    await expect(db.insert(playerPortalStates).values({ campaignId: ids.campaignB, playerId: ids.playerA, status: {} })).rejects.toThrow();
  });
  ...
  it("deletes user-specific grants when the user is deleted", async () => {
    ...
    const grants = await db.select().from(schema.visibilityGrants).where(eq(schema.visibilityGrants.targetId, "entity_for_viewer"));
    expect(grants).toHaveLength(0);
  });
});
