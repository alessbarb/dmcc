...
describe("visibility grants", () => {
  it("stores independent grants for different users on the same target", async () => {
    ...
    await db.insert(schema.visibilityGrants).values([
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_shared", scope: "specific_user", source: "manual", userId: ids.userA, playerId: null },
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_shared", scope: "specific_user", source: "manual", userId: ids.userB, playerId: null },
    ]);
  ...
  it("removes one user's grant without affecting another user", async () => {
    await seedVisibilityFixture();
    await db.insert(schema.visibilityGrants).values([
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_character", scope: "specific_user", source: "manual", userId: ids.userA, playerId: null },
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_character", scope: "specific_user", source: "manual", userId: ids.userB, playerId: null },
    ]);
  ...
  it("keeps player grants independent on the same target", async () => {
    await seedVisibilityFixture();
    await db.insert(schema.visibilityGrants).values([
      { campaignId: ids.campaign, targetType: "fact", targetId: "fact_shared", scope: "specific_player", source: "manual", userId: null, playerId: ids.playerA },
      { campaignId: ids.campaign, targetType: "fact", targetId: "fact_shared", scope: "specific_player", source: "manual", userId: null, playerId: ids.playerB },
    ]);
  ...
  it("revokes stale visibility grants without deleting explicit grants", async () => {
    await seedVisibilityFixture();
    await db.insert(schema.visibilityGrants).values([
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_stale", scope: "all_players", source: "visibility", userId: null, playerId: null },
      { campaignId: ids.campaign, targetType: "entity", targetId: "ent_linked", scope: "specific_user", source: "character_link", userId: ids.userA, playerId: null },
    ]);
    ...
    expect(grants).toContainEqual(expect.objectContaining({ targetId: "ent_linked", source: "character_link", userId: ids.userA }));
  });
});
