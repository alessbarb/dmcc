...
function group(
  ...
  return { id, kind: "group" as NodeKind, title, x, y, width: w, height: h, color };
}
...
export async function seedCanvas() {
  ...
  await api("POST", `/api/campaigns/${CMP}/canvases`, {
    ...
    kind: "mystery",
    description: "Las 21 pistas organizadas en 4 fases narrativas con sus conexiones a los 15 secretos DM.",
  });
  console.log("✓ Created canvas: Cadena de Investigación");
  await seedMysteryCanvas(cvs3Id);

  console.log("\n✓ All canvases seeded successfully (3 boards, full campaign coverage)");
}
